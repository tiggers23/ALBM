            ...
            if len(compound_prompts_deeper) > 0:
                ...
                if not self.text_layer:
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        ...
                        visual_context = visual_context.expand(x.shape[1], -1, -1).permute(1, 0, 2).half()      #先复制为[bs, 2, 768],后改变为[2, bs, 768]
...
class ResidualAttentionBlock_Attribute(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, design_details=None,
                 text_layer=False, i=0):
        super().__init__()
    ...
    def attention(self, x: torch.Tensor, counter=None, attnweight=None):
        
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        if attnweight is None:
            return self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask)[0]
        elif attnweight == True:
            x, attn_map = self.attn(x, x, x, need_weights=True, attn_mask=self.attn_mask)
            
            # attn_map = attn_map.detach().cpu().numpy()
            # path = './attn_map/num_{}_layer_{}'.format(self.batch, counter)
            # if not os.path.exists(path):
            #     # self.batch += 1
            #     # path = './attn_map/num_{}_layer_{}'.format(self.batch, counter)
            #     np.save(path, attn_map)
            return x[0]
        
    
    def forward(self, inputs, attnweights=True):
        # For the first layer, we do not need to add any duplicate, as it is already added
        # as the shallow version
        x = inputs[0]
        compound_prompts_deeper = inputs[1]
        ...
        if not self.text_layer:
            if self.first_layer:
                visual_context = inputs[1]
                #visual_context = visual_context.expand(x.shape[1], -1, -1).permute(1, 0, 2).half()      #先复制为[bs, 2, 768],后改变为[2, bs, 768]
                visual_context = visual_context.unsqueeze(0).expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                # Add the learnable tokens of this layer with the input, by replacing previous
                # layer learnable tokens
                ...
                # x = x + self.attention(self.ln_1(x), counter, attnweights) ##########################get attn map

                x = x + self.mlp(self.ln_2(x))
                # x = x + self.mlp(self.ln_2(x)) + LoRa_th(x) ################################add LoRA
    ...
    def forward(self, inputs, attnweights=True):
        ...
        if not self.text_layer:
            ...
            LoRa = inputs[2]
            LoRa_th = LoRa[counter_tensor.long()]
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT':
            ...
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_MaPLe(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
            print("MaPle use Attribute transformer!")
            
            # self.resblocks = nn.Sequential(
            #     *[ResidualAttentionBlock_Attribute(width, heads, attn_mask, design_details, text_layer, i)
            #       for i in range(layers)])
        ...
        ###############################################################
    def forward(self, x: torch.Tensor, attnweights=None):
        if attnweights is None:
            return self.resblocks(x)
        elif attnweights == True:
            return self.resblocks(x)        #self.resblocks(x, attnweights)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        return x
    '''
    def forward(self, x: torch.Tensor, num_attribute=None, LoRa=None):
        #print("use VisionTransformer Attribute")
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat(
        ...
        outputs = self.transformer([x, 0, LoRa])  # second argument is counter
        x = outputs[0]
        x = x.permute(1, 0, 2)  # LND -> NLD

        if num_attribute != None:
            prompt_all = self.ln_post(x[:, -(num_attribute):, :])
            img_all    = self.ln_post(x[:, 0, :])
            if self.proj is not None:
                prompt_all = prompt_all @ self.proj
                img_all    = img_all    @ self.proj

            return prompt_all, img_all
        else:
            img_all = self.ln_post(x[:, 0, :])
...
class VisionTransformer_Attribute(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
                 design_details, num_attribute=None):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=width, kernel_size=patch_size, stride=patch_size, bias=False)
        ...
        ####################################################   attention mask
        self.patches = (input_resolution // patch_size) ** 2 + 1
        self.num_attribute = num_attribute
        if self.num_attribute is None:
            print('no mask')
        else:
            self.mask = self.build_attention_mask()



        self.prompt_till_layer_visual = 0
        self.transformer = Transformer(width, layers, heads, attn_mask=self.mask,design_details=design_details)
        #self.transformer = Transformer(width, layers, heads, design_details=design_details)
        self.ln_post = LayerNorm(width)
        self.proj = nn.Parameter(scale * torch.randn(width, output_dim))
    ...
    def build_attention_mask(self):
        ...
        mask = torch.zeros(self.patches + self.num_attribute, self.patches + self.num_attribute)        # 13 = len of attribute
        #mask = torch.empty(13, 13)
        #mask.fill_(float("-inf"))
        #mask.triu_(1)  # zero out the lower diagonal
        
        for i, row in enumerate(mask):
            for j, element in enumerate(row):
                if i > -1 and j > (self.patches-1):
                    if i != j:
                        mask[i][j] = float("-inf")

                if i > (self.patches-1) and j == 0:
                    mask[i][j] = float("-inf")

######################################################################## mask vision prompt
        # for i, row in enumerate(mask):
        #     for j, element in enumerate(row):
        #         if i > (self.patches-1) and j > (self.patches-1):
        #             if i != j:
        #                 mask[i][j] = float("-inf")
        ...
        #print("don't see each other, visual see prompt!!!")
        #print("see each other all !!!")
        #print("prompt don't see each other and visual don't see prompt!!!")
        print("prompt don't see each other and visual don't see prompt, cls token after!!!")
        
        print("shape of mask:", mask.shape)
        return mask

    def forward(self, x: torch.Tensor, compound_deeper_prompts, num_attribute=None, LoRa=None,attnweight=None):
        #print("use VisionTransformer Attribute")
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        ...
        # After positional embeddings, we will attach prompts with the model, remember only those
        # are trainable parameters here in whole image encoder.
        #x = torch.cat([x, compound_deeper_prompts])
        #print('before: ', x[:, 0, :])
        ...
        #print('before prompts', compound_deeper_prompts[0, :])
        compound_deeper_prompts = self.ln_pre(compound_deeper_prompts)
        #print('after prompts', compound_deeper_prompts[0, :])
        x = x.permute(1, 0, 2)  # NLD -> LND
        # Again combine the inputs, so nn.sequential can work
        #outputs = self.transformer([x, compound_deeper_prompts, 0])  # third argument is counter
        #x = outputs[0]
        ...
        if attnweight is None:
            ...
            outputs = self.transformer([x, compound_deeper_prompts, 0], attnweight)
            x = outputs[0]
            x = x.permute(1, 0, 2)
...
class VisionTransformer_weight_matrix(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
                 design_details):
        super().__init__()
        self.input_resolution = input_resolution
    ...
    def forward(self, x: torch.Tensor):
        ...
        #x = outputs[0]
        #x = x.permute(1, 0, 2)  # LND -> NLD
        outputs = self.transformer(x)  # third argument is counter
        x = outputs
        x = x.permute(1, 0, 2)  # LND -> NLD
    ...
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "MaPLe":
                ...
                print('num_attribute 2: ', num_attribute)
                
                self.visual = VisionTransformer_Attribute(
                    input_resolution=image_resolution,
                    patch_size=vision_patch_size,
                    width=vision_width,
                    layers=vision_layers,
...
def build_model(state_dict: dict, design_details, num_attribute=None):
    ...
    if vit:
        ...
        #print("vsion_patch_size:", vision_patch_size)
    ...
    model = CLIP(
        ...
        context_length, vocab_size, transformer_width, transformer_heads, transformer_layers, design_details, num_attribute
    )

    print("**********************************************************************")
    print("vision_layers:", vision_layers)
    print("type:", type(vision_layers))
